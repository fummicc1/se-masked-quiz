...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`proposals_proposal_id_idx\` ON \`proposals\` (\`proposal_id\`);`)
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`quiz_answers_proposal_id_idx\` ON \`quiz_answers\` (\`proposal_id\`);`)
