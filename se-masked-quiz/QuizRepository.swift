...
actor QuizRepositoryImpl: QuizRepository {
    ...
    init(
        ...
        Task {
            do {
                func update(run: (isolated QuizRepositoryImpl) async throws -> ()) async throws {
                    try await run(self)
                }
    ...
    func getAllScores() async -> [String: ProposalScore] {
        guard let data = userDefaults.data(forKey: Self.scoreKey),
              let scores = try? JSONDecoder().decode([String: ProposalScore].self, from: data) else {
            return [:]
        }
        return scores
    }
...
private extension QuizRepositoryImpl {
